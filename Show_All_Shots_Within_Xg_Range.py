...
def seasonid(leagueid):
    ...
    for t in range(0, len(seasons)):
        add_website_if_valid(
            f"http://www.sofascore.com/api/v1/unique-tournament/{leagueid}/season/{seasons[t]['id']}/events/round/1",
            src)
    for i in range(0, len(src)):
        id = seasons[i]['id']
        if 'hasXg' in fetch_and_parse_json(
                f"http://www.sofascore.com/api/v1/unique-tournament/{leagueid}/season/{id}/events/round/1")['events'][
            0]:
            options.append(seasons[i]['year'])
            print(f"{i + 1}. {options[len(options) - 1]}")
...
def match_list(seasonid, leagueid, round):
    ...
    for p in range(0, int(round)):
        roundmatches = fetch_and_parse_json(
            f"http://www.sofascore.com/api/v1/unique-tournament/{leagueid}/season/{seasonid}/events/round/{p + 1}")[
            "events"]
...
def roundcalc(leagueid, seasonid):
    teamnum = len(fetch_and_parse_json(
        f"http://www.sofascore.com/api/v1/unique-tournament/{leagueid}/season/{seasonid}/standings/total")['standings'][
                      0]['rows'])
    return (teamnum - 1) * 2


def shotmap_data(matchid):
    event_ = fetch_and_parse_json(f"http://www.sofascore.com/api/v1/event/{matchid}")['event']
    if event_.get('status') != None:
        if event_['status']['type'] != 'postponed' and check_website(
                f"http://www.sofascore.com/api/v1/event/{matchid}/shotmap"):
            return fetch_and_parse_json(f"http://www.sofascore.com/api/v1/event/{matchid}/shotmap")['shotmap']
    elif check_website(f"http://www.sofascore.com/api/v1/event/{matchid}/shotmap"):
        return fetch_and_parse_json(f"http://www.sofascore.com/api/v1/event/{matchid}/shotmap")['shotmap']
    ...
    for i in range(0, len(matches)):
        ...
        if shotmap != None:
            for j in range(0, len(shotmap)):
                item = shotmap[j]
                biglist.append((nonround(item.get('xg'), 2), nonround(item.get('xgot'), 2),
                                item.get('playerCoordinates'), item.get('goalMouthCoordinates'), item.get('draw'),
                                item.get('blockCoordinates'), item.get('shotType')))
    return biglist
...
def plot_shots(xg_min, xg_max):
    ...
    response = input("Would you like to see the shot trails? (y/n) ")
    if response == 'y':
        for shot in shots:
            ...
            if xg != None:
                if xg_min <= nonround(xg, 2) <= xg_max:
                    ...
                    if 0 <= shot_x <= 75 and 0 <= shot_y <= 100 and shot_type == 'goal':
                        ...
                        ax.plot([startdrawy, enddrawy], [75 - startdrawx, 75 - enddrawx], color='red', linewidth=2,
                                alpha=0.3)
                        ...
                        ax.plot([startdrawy, blockdrawy], [75 - startdrawx, 75 - blockdrawx], color='white', linewidth=1,
                                alpha=0.3)
                    elif 0 <= shot_x <= 75 and 0 <= shot_y <= 100:
                        misscount += 1
                        # Plot the shot, with shot_x becoming y-axis and shot_y becoming x-axis
                        ax.scatter(shot_y, 75 - shot_x, color='white', s=100, alpha=0.6, edgecolor='black')
                        ax.plot([startdrawy, enddrawy], [75 - startdrawx, 75 - enddrawx], color='white', linewidth=1,
                                alpha=0.3)
    else:
        for shot in shots:
            xg, xgot, player_coords, shot_coords, draw_info, block_coords, shot_type = shot
            ...
            if xg != None:
                if xg_min <= nonround(xg, 2) <= xg_max:
                    ...
                    if 0 <= shot_x <= 75 and 0 <= shot_y <= 100 and shot_type == 'goal':
                        ...
                        # Plot the shot, with shot_x becoming y-axis and shot_y becoming x-axis
                        ax.scatter(shot_y, 75 - shot_x, color='white', s=100, alpha=0.6, edgecolor='black')
                        ax.plot([startdrawy, blockdrawy], [75 - startdrawx, 75 - blockdrawx], color='white',
                                linewidth=1,
                                alpha=0.0)
                    elif 0 <= shot_x <= 75 and 0 <= shot_y <= 100:
                        misscount += 1
    ...
    plt.text(50, -7.5,
             f"Goals scored = {goalcount}      Goals Missed = {misscount}       Score rate = {round(goalcount / (goalcount + misscount), 4)}%",
